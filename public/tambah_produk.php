...
if (!cekUser()) {
    ...
    if ($select->num_rows > 0) {
        ...
        $_SESSION['id_apotek'] = $data['id_apotek'];
    }
}


if (isset($_GET['submit-produk'])) {

    print_r($_FILES);
}

    // // Cek apakah terdapat error pada file yang diupload
    // if ($_FILES['berkas']["error"] == UPLOAD_ERR_OK) {
    //     // Siapkan nama file yang akan disimpans
    //     $tmp_name = $_FILES['berkas']["tmp_name"];
    //     $name = basename($_FILES['berkas']["name"]);

    //     // Pindahkan file dari folder sementara ke folder tujuan upload
    //     move_uploaded_file($tmp_name, "$upload_dir/$name");

    //     $gambar = "assets/product/".$name;
    //     $id_apotek = $_SESSION['id_apotek'];

    //     $query = mysqli_query($con, "INSERT INTO produk (`id_apotek`,`id_kategori`,`nama_produk`,`deskripsi`,`harga`,`satuan`,`komposisi`,`dosis`,`aturan_pakai`,`gol_produk`,`gambar`,) VALUE ('$id_apotek','$kategori','$nama','$deskripsi','$harga','$satuan','$komposisi','$dosis','$aturan','$gol','$gambar')");
                    ...
                    <div class="w-[50%] mr-5 h-[100%]">
                        <form method="GET" enctype="multipart/form-data">
                            <div class="flex mr-6 mb-10 items-center">
                                ...
                                <input type="nama" id="nama" name="nama" class="border-2 ml-5 w-full py-2 px-3 bg-gray-200 text-slate-600 leading-tight rounded-md focus:outline-none focus:shadow-outline-blue-500" placeholder="Masukkan nama produk" required/>
                            </div>
                            <div class="flex mr-6 mb-10 items-center">
                                <label for="nama" class="flex text-slate-600 justify-end text-sm font-normal mb-2 ">
                                    Harga
                                </label>
                                <input type="number" required id="nama" name="harga" class="border-2 ml-5 w-full py-2 px-3 bg-gray-200 text-slate-600 leading-tight rounded-md focus:outline-none focus:shadow-outline-blue-500" placeholder="Masukkan harga produk" />
                            </div>
                            <div class="flex mr-6 mb-10 items-center">
                                <label for="email" class="flex text-slate-600 justify-end text-sm font-normal mb-2 mr-5">
                                    Satuan
                                </label>
                                <select required class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded-md leading-tight focus:outline-none focus:bg-white focus:border-gray-500" name="satuan">
                            ...
                            <div class="flex mr-6 mb-10 items-center">
                                ...
                                <textarea class="bg-gray-200 rounded-md focus:outline-none focus:bg-white py-2 px-4 block w-full leading-5 appearance-none" placeholder="Tuliskan deskripsi produk..." name="deskripsi"></textarea>
                            </div>
                            <div class="flex mr-6 mb-10 items-center">
                                <label for="email" class="flex text-slate-600 justify-end text-sm font-normal mb-2 mr-5">
                                    Kategori
                                </label>
                                <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded-md leading-tight focus:outline-none focus:bg-white focus:border-gray-500" name="kategori">
                                    <option>Pilih salah satu</option>
                                    <?php
                            ...
                            <div class="flex mr-6 items-center mb-10">
                                <label for="email" class="text-slate-600 justify-end text-sm font-normal mr-5 ">
                                    Komposisi
                                </label>
                                <textarea class="bg-gray-200 rounded-md focus:outline-none focus:bg-white py-2 px-4 block w-full leading-5 appearance-none" placeholder="Tuliskan deskripsi produk..." name="komposisi"></textarea>
                            </div>
                            <div class="flex mr-6 items-center mb-10">
                                <label for="email" class="text-slate-600 justify-end text-sm font-normal mr-5 ">
                                    Dosis
                                </label>
                                <textarea class="bg-gray-200 rounded-md focus:outline-none focus:bg-white py-2 px-4 block w-full leading-5 appearance-none" placeholder="Tuliskan deskripsi produk..." name="dosis"></textarea>
                            </div>
                            <div class="flex mr-6 items-center mb-10">
                                <label for="email" class="text-slate-600 justify-end text-sm font-normal mr-5 ">
                                    Aturan pakai
                                </label>
                                <textarea class="bg-gray-200 rounded-md focus:outline-none focus:bg-white py-2 px-4 block w-full leading-5 appearance-none" placeholder="Tuliskan deskripsi produk..." name="aturan"></textarea>
                            </div>
                            <div class="flex mr-6 items-center mb-10">
                                <label for="email" class="text-slate-600 justify-end text-sm font-normal mr-5 ">
                                    Golongan Produk
                                </label>
                                <textarea class="bg-gray-200 rounded-md focus:outline-none focus:bg-white py-2 px-4 block w-full leading-5 appearance-none" placeholder="Tuliskan deskripsi produk..." name="gol"></textarea>
                            </div>
                            <div class="flex mr-6 items-center mb-10">
                                <label for="email" class="text-slate-600 justify-end text-sm font-normal mr-5 ">
                                    Gambar
                                </label>
                                <input required type="file" name="berkas" class="block w-full rounded-md transition duration-150 ease-in-out bg-gray-200 px-2 py-1 sm:text-sm sm:leading-5" />
                            </div>
                            <div class="mr-6 mb-2 items-center">
                                <p class="text-red-500"><?= @$error ?></p>
                            </div>
                            <div class="mr-6 mb-10 items-center">
                                <button class="px-3 py-1 rounded-md text-white font-semibold bg-sky-500" name="submit-produk">Tambahkan</button>
                            </div>
